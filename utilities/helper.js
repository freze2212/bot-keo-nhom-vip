...
async function appendToLog(message, logsName) {
    ...
    try {
        const verbose = process.env.SERVER_VERBOSE_LOG === 'true';
        const important = /error|fail|warn|MAIN HO|SCREENSHOT FAIL|ENOSPC|disk is full|HẾT SESSION|restart/i.test(
            String(message || '')
        );
